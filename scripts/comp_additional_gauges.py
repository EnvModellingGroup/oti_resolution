...
def main():
    ...
    for name in tide_gauge_data:
        ...
        for times, model, label in zip(model_times, model_elev_data, labels):
            mod_ln = ax.plot(times[:n_times]/86400., model[name].to_numpy()[:n_times], color=colours[i], lw=1, label=label)
        ...
        if not stub is None:
            name = name +"_" + stub
        plt.savefig(os.path.join(output_dir,output,"comp_tidal_plot_"+name+".pdf"), dpi=180, bbox_inches = 'tight', pad_inches = 0)
        plt.close()
    ...
    for name in tide_gauge_data:
        ...
        if not stub is None:
            name = name +"_" + stub
        plt.savefig(os.path.join(output_dir,output,"comp_speed_plot_"+name+".pdf"), dpi=180, bbox_inches = 'tight', pad_inches = 0)
        plt.close()
