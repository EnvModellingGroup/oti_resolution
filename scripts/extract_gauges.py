...
def main():
    ...
    # fid last Elevation number rather than use the params to make sure we can deal with partial
    # runs
    folder = Path(os.path.join(thetis_dir,params.output_dir,"hdf5/"))
    highest = max(int(file.stem[-5:]) for file in folder.glob('Elevation*.h5'))
    t_end = highest * params.output_time
    ...
    print(len(thetis_times), np.shape(elev_data))
